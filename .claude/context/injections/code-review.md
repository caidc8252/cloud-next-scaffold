...
- **`*.stub` misuse** — a `*.stub.ts` imported by code → block (stubs are build inputs for `gen:coc`, never imported). A `*.stub.ts` still present after the owning module declares the code for real (stale — the coc `duplicate-code` build gate fires) → flag. `commons` with a non-null `menuCode`/`belongToMenuCode` or any call into a business module → flag.
