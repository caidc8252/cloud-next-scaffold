...
- Review surfaced findings? **Tell the operator to run `/logic-groom`** to capture them as fragments (`待处理`) that re-enter the groom→`/logic-analyze`→`/coding` loop (user-triggered, not yours to invoke) — do not hand-patch findings ad hoc.
