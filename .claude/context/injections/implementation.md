...
## When the code is party/owner-scoped (authorization negatives)
...
`require-e2e-cell` (eslint) forces a marker on every `route.ts` / `middleware.ts` — ship one or opt out with `/** @e2e-cell-skip reason=… */`. `kind=auth-boundary` isn't lint-forced — add it by hand. `pnpm check:e2e-orphans` (in `pnpm test:e2e`) then requires a matching `e2e/<feature>.spec.ts`.
