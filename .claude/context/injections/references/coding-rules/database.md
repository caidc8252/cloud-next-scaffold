...
- Every business table MUST carry `partyId Int @map("party_id")` and an index on it (`@@index([partyId])`, or a composite like `@@index([partyId, status])`). See `SysPartyUser`, `SysNotice` (`belongToPartyId`).
- Per-party uniqueness MUST be expressed as `@@unique([partyId, <key>])`. Example: `@@unique([partyId, userId], map: "uk_sys_party_user")`.

**Naming.** Keep key names identical across tables when they mean the same thing (no ambiguous duplication). Prisma fields are camelCase, columns snake_case via `@map` (`partyId → party_id`).
...
- **MUST NOT persist file info until the service layer has validated it** (permission, ownership, type, visibility, S3 `HeadObject`).
