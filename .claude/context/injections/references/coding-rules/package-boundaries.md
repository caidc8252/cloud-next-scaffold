...
- **`packages/*` MUST NOT import from `apps/`.** Dependency flows one way (`apps/web` → `@cloud/*`, never back) (structural, not a lint rule).
- **`@cloud/*` holds only project-level shared infrastructure** — capabilities reused across apps. App-private or single-page logic stays in `apps/web` (`modules/`, commons, or `apps/web/lib/`); don't pull a business concern into a shared package. What earns promotion into a package: cross-app reuse with a clean responsibility boundary. A two-ended capability (client + server) is promoted **whole**, with `./client` / `./server` dual entries (`client-only` / `server-only` guards) — never split one end into `apps/*`.
