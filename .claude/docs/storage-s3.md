...
> 归属：`@cloud/storage` 上传/下载、`storage_object`、bucket policy。**动任何 S3 / 上传下载前必读。**
...
- 文件业务归属不要塞进 `storage_object`；单值强关系在业务表存 `storageObjectId`，多文件集合在业务表存 `storageObjectId[]`，数组顺序就是展示顺序
- 公开图片使用 `storage_object.visibility = PUBLIC`，只允许 `image/*`，且对象 key 必须落在 `public/` 前缀；业务接口可按自身 DTO 约定返回公开文件的可访问 URL
