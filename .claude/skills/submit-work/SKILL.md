...
4. **PR 决策（默认选中「不提交」）** —— 询问操作员是否把工作 PR 到 `develop`：
   ...
   - **提交** → 对**三个仓库**各自的 `feature/task-<task_id>` 开 PR 到 `develop`：
     ...
     - **需求空间** `../pep-webapp-docs`、**数据模型空间** `../pep-data-model-docs`：
       - **兜底 `/logic-converge` 可能被跳过**：本地无该仓 → 先按各文档仓自描述 README 的 URL `git clone` 到 `../pep-webapp-docs` / `../pep-data-model-docs`；若该仓不在 `feature/task-<task_id>`（分支不存在则基于最新 `develop` 创建），先 checkout 到它；有未提交的回写改动 → `git add -A` + `commit`（信息含 `task_id`）。
