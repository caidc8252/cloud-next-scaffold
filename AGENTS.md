...
5. **`/coding`**（构建入口兼**预检路由**：缺 sync/converge/原型时会话内引导补齐再构建）— 消费**全部输入 + `logic.md`**（后者是补充件，非唯一来源），起 superpowers 流水线（`writing-plans → executing-plans`）生成/改模块（`gen:coc` + 测试全绿）；流水线 review 的 findings → 人工触发 **`/logic-groom`** 回灌为碎片（`待处理`），走 groom→converge→code 闭环重新消费，不在此就地 debug。不回写飞书、不开 PR（那是 `/submit-work`）。
