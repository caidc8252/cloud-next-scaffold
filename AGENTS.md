...
4. **`/logic-analyze`** — 读「需求 specs + 原型 + 数据模型 + 现有代码 + groom 碎片」，把「两份真理都没说、但写代码必须知道」的实现逻辑沉淀成 `logic.md`（经 `ledger.mjs` 渲染，是 coding 直读的**四真源之一**：specs + 原型 + data-model + logic.md，**非唯一交接物**），并回写 workbench 文档基线。**只读** `apps/web/commons/<mod>/overview.md` 与各模块 `overview.md` 做全局认识——这些 `overview.md` 由专门的 **commons 维护 skill** 生成/维护（建设中），`/logic-analyze` 不写入它们。
5. **`/coding`** — **直读四真源**（specs + 原型 + data-model + logic.md，不只读 logic.md），起 superpowers 流水线（`writing-plans → executing-plans`）按**分相流程**（读真理→覆盖清单 / 地基生成 / 地基硬检查点〔含操作员在线看 UI〕/ 逻辑生成+收敛 / 终检）生成/改模块，双检查（`coverage.mjs gate` 覆盖 + 语义对账）+ `gen:coc` + 测试全绿；流水线/对账 review 的 findings → 人工触发 **`/logic-groom`** 回灌为碎片（`待处理`），走 groom→analyze→code 闭环重新消费，不在此就地 debug。不回写飞书、不开 PR（那是 `/submit-work`）。
