...
- **`system/users` 与 `system/roles` 的 master-detail 工作台布局是 portal-page-style-spec §0.3 的已记录偏离**：这两页不走 §1.1 标准列表页（PageHeader + 条件栏 + Table + 分页、详情走子路由），而用「左列表 pane + 右详情/编辑 pane」的工作台形态——roles 的权限树需要内联编辑，users 选中即看详情，左右同屏比「列表→子路由详情」更高效。其余样式全部对齐规范基线：`PageHeader` + `PageBody`、`Badge`（tone+dot / shape=tag）、销毁动作 `danger`、`ToggleGroup variant="plain"` 选择态磁贴、无渐变、尺寸/间距吸附档位。左 pane 用 `sticky top-6 self-start` + `max-h-[calc(100dvh-9rem)]` 做 pane 内滚动——规范无工作台 pane 滚动的 token，这一处 `dvh` 计算是该偏离下唯一允许的任意值。新增同类管理页默认仍走标准列表页范式，不要无理由复制这套工作台布局。
...
- **锁定与状态解耦**：`SysUser.status` 只表达账号级（仅 `ACTIVE` 放行）；刷错锁只写 `passwordErrorLockExpiredTimestamp`（不再写 `status=LOCKED`）。阈值/时长走 env（`AUTH_PASSWORD_MAX_ERROR_TIMES` 默认 6，`errorTimes >= 6` 上锁；`AUTH_PASSWORD_LOCK_MINUTES` 默认 30）。只有密码正确才清零；锁过期后再错立即重新上锁且次数继续累加。登录决策逻辑在 `apps/portal/lib/login-checks.ts`；web 保留同名时间窗 helper 给修改密码接口复用。
...
- **portal 统一登录与 partner 选择**：当前登录页、登录 API、MFA 和 partner 选择在 `apps/portal`。登录成功按可选 partner 聚合：只有一个可选 partner 时直接创建完整 session 并跳到 `WEB_APP_URL`；多个或没有可选 partner 时进入 portal `/select-partner` 显示不可选原因。选择 partner 时，`apps/portal/lib/session-snapshot.ts` 用全局 manifest + DB 角色/契约直接生成完整 session 并 `updateSession()`，然后跳到 `WEB_APP_URL`。web 的 `/login`、`/select-partner` 和 logout 会跳回 portal。
- **跨子域 cookie**：本地 `localhost:3000/3100` 可共享 host cookie；生产若是 `portal.example.com` / `web.example.com`，配置 `SESSION_COOKIE_DOMAIN=.example.com`，否则 portal 写入的 `sid` 不会发送给 web。
