...
- 与 `packages/*` 的区别：`packages/*` 是项目级共享基础设施（跨 app），`commons/` 是 `apps/web` 内由业务沉淀出的通用模块。新增共享能力先判断该沉淀到哪一层（见 `.claude/context/injections/references/coding-rules/package-boundaries.md`）。
