...
// 重置密码一次性 token。key/value 形状是跨 app 契约：
// `auth:pwreset:{token}` = { userId, source }。256-bit 不可猜、一次性。
// TTL 按 source：自助找回短（60m）；管理员触发长（72h，admin 侧签发）。
export type ResetTokenSource = "self-service" | "admin";
type ResetTokenEntry = { userId: number; source: ResetTokenSource };

const ADMIN_TTL_SECONDS = TTL.AUTH_PW_RESET_ADMIN_SECONDS;
...
export async function createPasswordResetToken(userId: number): Promise<string> {
  ...
  await kv.set(tokenKey(token), { userId, source: "admin" } satisfies ResetTokenEntry, ADMIN_TTL_SECONDS);
  return token;
}
