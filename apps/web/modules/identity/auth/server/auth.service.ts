...
// auth 域业务编排：登录（含刷错锁 / RSA 解密 / nonce 防重放 / MFA 分岔）、MFA 二次校验、选择公司。
// 登录入口统一在本 app；成功后跨 host handoff（方案 B）让目标 console host 写 sid cookie。
...
/** 带 returnTo 的登录收尾（onboarding 用）：只建本 app 会话（不跨 host handoff），跳回站内 returnTo。
