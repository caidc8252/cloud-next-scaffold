...
4. **#3 重置密码(admin)** —— ✅ **后端已落地**:admin `lib/email/password-reset.ts` + `sendResetLinkEmail`(链接落 portal `/reset-password?token=`,`purpose=password-reset`);`createPasswordResetToken` 加 `source:"admin"`(与 portal 共享 `pwreset:{token}` 契约,72h);`resetUserPassword` 接上发信。消费端复用 portal(已 ②)。**仅剩共享重置页(前端 ④)**。
