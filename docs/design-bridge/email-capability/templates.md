...
3. **#4 忘记密码(portal)** —— ✅ **完成**（已与 #3 统一为 token 链接）:`send-link` 发重置链接邮件（`password-reset` 模板）+ portal `/reset-password?token=` 消费页（`validate` → RSA 设密 → 历史去重 → 清锁）；前端 `forgot-screen`（邮箱→查邮件）+ 重置页均已落地。`lib/password-input` 与 onboarding 共用。详见 `reset-password/`。
4. **#3 重置密码(admin)** —— ✅ **后端已落地**:admin `lib/email/password-reset.ts` + `sendResetLinkEmail`(链接落 portal `/reset-password?token=`,`purpose=password-reset`);`createPasswordResetToken` 加 `source:"admin"`(与 portal 共享 `pwreset:{token}` 契约,72h);`resetUserPassword` 接上发信。消费端 + 重置页复用 portal(②④,已落地) → **端到端可用**。
