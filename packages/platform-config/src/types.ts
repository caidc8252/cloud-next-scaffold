...
/** 单条权限定义。label/desc 为 i18n key（相对 coc 命名空间）。 */
export type MenuPermission = {
  code: string;
  label?: string;
  desc?: string;
  /** 同菜单内的前置权限 code；无前置为 null/省略。授权 UI 据此链式联动。 */
  require?: string | null;
};
 ...
 * `permissionCodes` 必须是菜单池里出现过的 permissionCode；预置超管也**显式列全**（不再留空自动填充），
 * 完整性由非阻断脚本 check:roles 兜底。
 * `roleName` / `remark` 为 i18n key（相对 coc 命名空间），必填。
 */
export type RoleDef = {
  roleId: number;
  roleName: string;
