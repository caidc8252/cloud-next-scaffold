...
// Structure mirrors the shadcn input-group registry component; classes carry the TOMS re-skin plus the team's
// combobox/disabled integration (see the grouped comments below).
function InputGroup({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      role="group"
      className={cn(
        "group/input-group relative flex h-control-md w-full min-w-0 items-center rounded-md border border-input outline-none transition-colors",
        ...
        // TOMS additions beyond upstream — combobox integration: inside combobox-content the group defers
        // its border/ring to the popover so the field doesn't double up the focus treatment.
...
const inputGroupAddonVariants = cva(
  "flex h-auto cursor-text items-center justify-center gap-2 py-1.5 text-md font-medium text-muted-foreground select-none group-data-[disabled=true]/input-group:opacity-50 [&>kbd]:rounded-[calc(var(--radius)-5px)] [&>svg:not([class*='size-'])]:size-4",
  {
    variants: {
      align: {
        "inline-start":
...
const inputGroupButtonVariants = cva(
  "flex items-center gap-2 text-md shadow-none",
  {
    variants: {
      size: {
...
function InputGroupText({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      className={cn(
        "flex items-center gap-2 text-md text-muted-foreground [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    />
