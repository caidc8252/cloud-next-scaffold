...
export const Pagination: React.FC<PaginationProps> = ({
  ...
  const pageBtn = (
    ...
    opts?: {
      ...
      key?: string | number
      ariaLabel?: string
      /** Page-number buttons get mono/tabular digits so they line up; arrows don't. */
      mono?: boolean
    }
  ) => (
    <button
      ...
      className={cn(
        'inline-flex items-center justify-center min-w-7 h-control-md px-2 rounded-md',
        'border text-md cursor-pointer',
      ...
      {pages.map((p, i) =>
        p === '…' ? (
          <span
            key={`e${i}`}
            aria-hidden
            className="inline-flex items-center justify-center min-w-7 h-control-md px-2 border border-transparent text-md text-content-secondary select-none"
          >
            …
          </span>
        ) : (
          pageBtn(p, () => onChange(p), { active: p === page, key: p, mono: true })
        )
      )}
