...
describe("Table — TOMS v2.0 variants", () => {
  ...
  it("header uses the TOMS DS 2.0 overline: weight 500, uppercase, overline tracking", () => {
  ...
  it("numeric column right-aligns and renders the body cell in mono/tabular digits", () => {
    const numericCols: TableColumn<Row>[] = [
      { key: "id", title: "ID", field: "id", numeric: true },
      { key: "name", title: "Name", field: "name" },
    ]
    render(<Table columns={numericCols} rows={ROWS} rowKey={(r) => r.id} />)
    ...
    expect(idCell?.style.textAlign).toBe("right")

    // Header right-aligns too.
    expect(screen.getByText("ID").closest("th")?.style.textAlign).toBe("right")

    // Non-numeric column is untouched: left-aligned, no mono.
    const nameCell = screen.getByText("alpha").closest("td")
    expect(nameCell?.className).not.toContain("font-mono")
    expect(nameCell?.style.textAlign).toBe("left")
  })

  it("explicit align on a numeric column wins over the numeric right-align default", () => {
    const numericCols: TableColumn<Row>[] = [
      { key: "id", title: "ID", field: "id", numeric: true, align: "center" },
      { key: "name", title: "Name", field: "name" },
    ]
    render(<Table columns={numericCols} rows={ROWS} rowKey={(r) => r.id} />)
    const idCell = screen.getByText("1").closest("td")
    // align overrides direction, but mono/tabular still applies.
    expect(idCell?.style.textAlign).toBe("center")
    expect(idCell?.className).toContain("font-mono")
  })
})
