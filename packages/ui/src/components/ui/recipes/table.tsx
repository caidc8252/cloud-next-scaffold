...
export interface TableColumn<R> {
  ...
  /**
   * Numeric column: right-aligns the th/td (unless an explicit `align` is set)
   * and renders the body cell in `font-mono tabular-nums` so digits line up.
   */
  numeric?: boolean
}
...
export function Table<R>({
  ...
  // Numeric columns right-align unless the column sets an explicit align.
  const colAlign = (col: TableColumn<R>): 'left' | 'right' | 'center' =>
        ...
        <thead className="bg-surface-3 sticky z-10" style={{ top: stickyHeaderTop ?? 0 }}>
          <tr>
            {columns.map((col, colIndex) => (
              <th
                key={col.key}
                style={{ width: col.width, textAlign: colAlign(col) }}
                className={cn(
                  HEAD_DENSITY[density],
                  // TOMS DS 2.0 thead: 12px / weight 500 / UPPERCASE / tertiary / surface-3 / sticky.
                  'text-xs font-medium uppercase text-content-tertiary tracking-overline border-b border-line-default',
                  separator,
